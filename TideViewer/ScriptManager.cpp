...
ScriptManager::ScriptManager() : handle(NULL)
{
}
...
void ScriptManager::getFloat(float& v, const CStringW& name, float default_value)
{
    HSQUIRRELVM vm = static_cast<HSQUIRRELVM>(handle);
    v = default_value;
...
void ScriptManager::getFloat(std::vector<float>& v, const CStringW& name)
{
    HSQUIRRELVM vm = static_cast<HSQUIRRELVM>(handle);
    v.clear();
    ...
    if (SQ_SUCCEEDED(sq_get(vm, -2)))
    {
        ...
        if (ObjectType == OT_ARRAY)
        {
            ...
            {
                if (SQ_SUCCEEDED(sq_getfloat(vm, -1, &val)))
                {
