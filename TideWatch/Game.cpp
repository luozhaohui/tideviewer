...
bool Game::init(HWND _hWnd)
{
	...
	backBmp = CreateCompatibleBitmap(hdcFront, mainRect.right - mainRect.left,
...
void Game::loadData(const CStringW& dateString, const CStringW& place)
{
	...
	if (sptData.size() < 33){
		info.Format(L"无法从文件%s\\%s中\n正确读取潮高数据[%s]，\n请检查文件中的[%s]数据是否正确！", sptPath, fileName, curItem, curItem);
	...
	for (; it != end; it++){
		...
		CLog::output("position info - %02d:%02d(%d) : (%3.2f, %3.2f)\n", 
			int(data.time/100.0f), ((int)data.time)%100, int(data.tide), pt.X, pt.Y);
	}
...
void Game::draw()
{
	...
	g.FillRectangle(&bkg, mainRect.left, mainRect.top, mainRect.right - mainRect.left, mainRect.bottom - mainRect.top);
	...
	if(loginFlag && tideData.size() > 0){
		if (wcslen(strPlace) > 0 && wcslen(strDate) >= 8){
			...
			{
				info.Format(L"%s年%s月%s日，%s潮高分布图", dateW.Mid(0,4), dateW.Mid(4,2),dateW.Mid(6,2), strPlace);
				//drawTideString(&g, gdiBigFont, crBlack, info, 120, yLen + 70, true);
				drawCenterString(&g, gdiBigFont, crBlack, info, yLen + 70);
			}
			else {
				info.Format(L"%s年%s月%s日，农历%s，%s潮高分布图", dateW.Mid(0,4), dateW.Mid(4,2),dateW.Mid(6,2), longLiW, strPlace);
			...
			if (int(currentTime) >= 0 && int(currentTime) <= 2400 && currentTide >= 0 && currentTide < (countTide -1) * TIDE_STEP){
				...
				if (isShowTime)
				{
					for(size_t i = 0; i < tideData.size() - 1; i++){
						...
						{
							int time = getLineFuncX(mouseTide,
								tideData[i].time, tideData[i].tide,
...
void Game::drawTideEllipse(Gdiplus::Graphics* g, const Pen *pen, Gdiplus::Brush * brush, const PointF &pt1, float r1, float r2)
{
	if (g){
...
void Game::drawTideString(Gdiplus::Graphics* g, Gdiplus::Font * font, Gdiplus::Color color, const CStringW& string, float x, float y, bool bAlignLeft /* = true */)
{
	drawString(g, font, color, string, x + zeroPoint.X, zeroPoint.Y - y, bAlignLeft);
}

void Game::drawString(Gdiplus::Graphics* g, Gdiplus::Font * font, Gdiplus::Color color, const CStringW& string, float x, float y, bool bAlignLeft /* = true */)
{
	if (!g || !font){
		return;
	}
...
bool Game::checkInputData(const CStringW& inputData)
{
	...
	if (values[0] < 0 || values[0] > 2400 ||
		values[2] < 0 || values[2] > 2400 ||
		values[4] < 0 || values[4] > 2400 ||
		values[6] < 0 || values[6] > 2400 ||
		values[1] < 0 || values[1] > 1000 ||
...
bool Game::checkInputDataEx(const CStringW& filePath, CStringW& place, CStringW& date, CStringW& resultInfo)
{
	...
	for(int i = 0; i < dayCount; i++)
	{
		if(tideData[0][i] < 1 || tideData[0][i] > 31
			|| tideData[1][i] < 1 || tideData[1][i] > 31
		){
			return false;
		}
	}
...
bool Game::inputDataEx(const CStringW& filePath, const CStringW& place2, CStringW& resultInfo)
{
	...
	resultInfo.Format(L"【成功】:成功从文件 \"%s\" 录入 %s.%s.%02d - %s.%s.%02d 期间 %d 天的潮高数据。"
		,filePath, year, month, tideData[0][0], year, month, tideData[0][dayCount - 1], dayCount);
	return true;
}